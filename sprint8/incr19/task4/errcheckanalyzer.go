...
// errorType переменная - интерфейсный тип error.
...
// ErrCheckAnalyzer переменная анлизатора.
var ErrCheckAnalyzer = &analysis.Analyzer{
	Name: "errcheck",                   //имя анализатора. Нужно указать валидный Go-идентификатор, так как он может использоваться в параметрах командной строки, URL и т. д.
	Doc:  "check for unchecked errors", // текст с описанием работы анализатора. Этот текст будет отображаться по команде help.
	Run:  run,                          // функция, которая отвечает за анализ исходного кода.
}

// run содержит основной код проверки.
