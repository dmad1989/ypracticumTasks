...
func (i *Identy) Next(r rune) bool {
	if unicode.IsDigit(r) || unicode.IsLetter(r) {
		i.buf = append(i.buf, r)
		return true
	}
	i.lexer.NewToken(TNumber, i.buf)
	i.lexer.SetState(StateMain)
	i.buf = i.buf[:0]
	return false
}
...
func main() {
	...
	lex.states = []State{&Main{lexer: &lex}, &Number{lexer: &lex}, &Identy{lexer: &lex}}
